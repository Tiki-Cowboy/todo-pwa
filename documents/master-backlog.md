...
| DONE-009 | 2026-07-21 Sprint Deployed | `firebase deploy --only hosting` — ships routing foundation, FEAT-028/029/030/031/032/033, and BUG-017 documentation (bug itself not yet fixed) live. No Cloud Function changes this sprint, so Functions were left untouched. The `migrateLegacyProjectTypes()` one-time migration will run automatically against the real account's Firestore data on next app load. |
...
| FEAT-011 | Daily Recurring Tasks | Originally shipped as a project-type flag; **migrated to task-level `type` in the 2026-07-21 sprint** (see FEAT-030 row) — see that entry for the current architecture. Daily tasks use `lastCompletedDate` instead of `completedAt` — check off resets each day automatically, no backend cron needed. Daily Checklist section on Today tab. |
...
| — | Client-side routing foundation | Added `react-router-dom`; ported Today/Projects/Reporting from tab-state to routes (`/today`, `/projects`, `/projects/:projectId`, `/reporting`) behind `<BrowserRouter>` in `main.jsx`. `NavBar`/`BottomNav` now use `NavLink`. Behavior-preserving; prerequisite for FEAT-029. Firebase Hosting already had the SPA rewrite (`**` → `/index.html`) needed for deep links. |
| FEAT-029 | Dedicated Individual Project View | New `ProjectDetailView.jsx` at `/projects/:projectId` — header (title, description, statusNote, status pill, category, ↻/⟳ type badge, progress bar), Open Tasks list, Completed section reusing `CompletedByDay`. Back arrow returns to `/projects`. Reusable row components (`TaskRow`, `DailyTaskRow`, `SubtaskRow`, `SubtaskAddRow`, `StatusPill`, `HubSpotLinks`, `ACCENT_COLORS`) exported from `ProjectCard.jsx` for reuse here. Also added a dedicated "open" icon button to each `ProjectCard` header as a second entry point into the page (Today's pill only surfaces projects with a due/overdue task). |
| FEAT-028 | Project Pill Navigation (Today → Dedicated Project View) | Today view's project pill (`TodayTaskRow` in `HomeTab.jsx`) is now a button that navigates to `/projects/:id`. No-op for project-less tasks (pill doesn't render). |
| FEAT-032 | Scrollable Project Sections + In-Section Sorting | Each category's project grid in `ProjectsTab` wrapped in a `maxHeight: 640px` / `overflowY: auto` container. Per-section A–Z / Due sort toggle (`SectionSortControl`); "Due" sorts by each project's nearest open-task due date. Sort choice + scroll position kept in a module-level object (`sectionPrefs`) so they survive navigating to a project detail page and back (component unmounts on route change). |
| FEAT-033 | Sticky Category Headers on Projects Tab | Category header (`WORK`, `PERSONAL`, etc.) is `position: sticky; top: 54px` (just below the top nav), page-level rather than per-section — resolves the open question in the original ticket: since FEAT-032 gave each section its own internal scroll, the header stays in normal page flow above it and pins as the outer page scrolls past that category. |
| FEAT-031 | Today View: Organization & Visual Hierarchy | "Group by Project" toggle above Due & Overdue — when on, each category further splits into project sub-groups (General last). Category headers bumped to semibold `text-primary` with a short gold underline bar; project sub-headers stay small/muted — creates a two-tier visual hierarchy either way. |
| FEAT-030 | Recurring Tasks ("Frequents") + task-level type architecture | Shipped in two passes same day: first as a project-type flag (`daily`/`frequent`, mirroring FEAT-011's original approach), then **reworked to task-level `type`** after a design discussion — recurrence is a per-task property, so forcing users into dedicated single-purpose projects was the wrong model. Final architecture: `task.type: 'standard' \| 'daily' \| 'frequent'` (default `'standard'`), fully independent of which project a task lives in — any project can now freely mix one-off, daily, and frequent tasks. `TaskTypeToggle.jsx` (new, shared 3-way control) used in `ProjectCard`'s quick-add, `ProjectDetailView`'s add-task row, and `TaskEditModal`. Frequent tasks carry a `recurrence` field (`{ type: 'weekly'\|'monthly'\|'interval', daysOfWeek, dayOfMonth, intervalDays, nextDueDate }`) computed by `src/lib/recurrence.js` (`initialDueDate`, `computeNextDueDate`, `describeRecurrence`); `RecurrencePicker.jsx` edits it, shown only when type is Frequent. `DailyTaskRow`/`FrequentTaskRow`/`TaskRow` (all exported from `ProjectCard.jsx`) now render per-task based on `task.type`, so one project's task list can mix all three row styles. Today tab's "Daily Checklist" and "Frequents" sections (`HomeTab.jsx`) group by `task.type` across *all* projects (previously scanned only projects flagged daily/frequent), each task's own `projectId` still used for the sub-header/grouping. `useDailyReset` simplified to filter `tasks` by `type === 'daily'` directly — no longer needs a project-ID set passed in. Project-level `type` field is no longer read anywhere; `ProjectModal`'s Standard/Daily/Frequent toggle was removed. **Migration:** `migrateLegacyProjectTypes()` in `firestore.js` runs once via a `useEffect` in `AppShell` on every load — finds any project still carrying a legacy `type` of `daily`/`frequent`, copies it onto that project's tasks (`type` field, only if the task doesn't already have one), and resets the project back to `standard`. Idempotent, so it's a no-op once a user's data has been migrated; verified against the seeded test account's pre-refactor "Weekly Chores" (frequent) project — its task correctly picked up `type: 'frequent'` and the project's badge disappeared. Frequent tasks still have no reset hook — visibility is purely `recurrence.nextDueDate <= today`, so a missed one stays visible (overdue) indefinitely by construction. **Follow-up same day:** Monthly recurrence extended to match Google Calendar's custom-recurrence patterns, after user feedback that "first Sunday of every month" (and multi-day patterns like "1st and 15th") weren't representable with a single day-of-month. `recurrence.monthlyMode: 'days' \| 'weekday'` — `'days'` now takes an array `daysOfMonth` (multi-select 1–31 grid in `RecurrencePicker`, soonest-upcoming day wins each cycle, still clamped to month-end for e.g. day 31 in February) and `'weekday'` adds `weekdayOrdinal` (1st/2nd/3rd/4th/last, `-1` = last) + `weekday` (0–6) for "Nth weekday of month" patterns, computed by new `nthWeekdayOfMonth`/`nextMonthlyWeekday` helpers in `recurrence.js`. Verified against 13 hand-checked date-math cases (ordinary Nth-weekday, "last Friday", multi-day rollover, day-31 clamping) before touching the UI. Caught and fixed a real bug in the same pass: switching to "On a weekday" without touching the ordinal/weekday dropdowns left `weekdayOrdinal`/`weekday` `undefined` in state (the dropdowns' displayed defaults were cosmetic-only, never committed via `onChange` unless the user changed them) — `computeNextDueDate`/`initialDueDate` silently produced a garbage date instead of erroring. Fixed by having `RecurrencePicker`'s mode-switch handler seed real default values immediately, plus defensive `?? 1` / `?? 0` fallbacks in `recurrence.js` itself. |
...
| ID | Title | Notes |
|----|-------|-------|
| BUG-017 | No Bootstrap Path for a Brand-New Account | On a fresh account with zero projects/tasks, the Projects tab renders nothing — the "New project" outline card only appears under a category that already has a project or standalone task in it (`categoryNames` in `ProjectsTab.jsx` is derived from existing content, not from the raw `categories` list). `TaskInput.jsx` — presumably the intended first-run entry point — is not imported anywhere in the app. Discovered while QA-testing FEAT-028/029 with a throwaway account; does not affect existing accounts that already have data. |
...
| Record-breaking | (dynamic name) | Most tasks completed in a 24-hour period — record to beat auto-adjusts each time it's broken |
| Best day-of-week | e.g. "Most productive Wednesday ever" | Tracked independently per day-of-week (7 rolling records) |
| Hot streaks | (dynamic name) | Most tasks completed in a single hour/day/week — volume-in-a-window, distinct from Consistency streak badges |
| Day streaks | (dynamic name) | X consecutive days with ≥1 task completed — lower bar than the "all dailies" Consistency badges above |
| Weekly pattern | e.g. "dishes every day this week," "everything on time this week" | Requires per-task-name or per-category pattern matching within a week window |
| Volume | (extends existing tiers) | Adds 250 / 2,500 / 5,000 to the existing 50/100/500/1,000 tiers |
| Annual awards | "Most productive year to date" | Rolling year-over-year comparison |
| Year in review | "Wrapped"-style annual recap | Totals, category breakdowns, notable stats — own dedicated view, not a badge-shelf entry, surfaced around year-end |

**Additional Acceptance Criteria (from FEAT-D handoff merge):**
...
- Reconcile naming overlap between existing "Consistency" streak badges and new "Day streaks" during implementation — keep both, clarify naming
